...
ClothoidCurve::ClothoidCurve(BaseCurve const &C)
  ...
  switch (C.type()) {
    ...
    case G2LIB_POLYLINE:
      G2LIB_ASSERT(false, "ClothoidList constructor cannot convert from: "
...
void ClothoidCurve::optimized_sample_internal(real_type s_begin,
                                              real_type s_end, real_type offs,
                                              real_type ds, real_type max_angle,
...
void ClothoidCurve::optimized_sample(real_type offs, int_type npts,
                                     real_type max_angle,
                                     vector<real_type> &s) const {
  s.clear();
...
void ClothoidCurve::bbTriangles_internal(real_type offs, vector<T2D> &tvec,
                                         real_type s_begin, real_type s_end,
                                         real_type max_angle,
                                         real_type max_size,
  ...
  for (int_type npts = 0; ss < s_end; ++npts) {
    ...
    real_type alpha =
        sss - ss;  // se angolo troppo piccolo uso approx piu rozza
...
void ClothoidCurve::bbTriangles(real_type offs, vector<T2D> &tvec,
                                real_type max_angle, real_type max_size,
                                int_type icurve) const {
  if (CD.kappa0 * CD.dk >= 0 || CD.kappa(L) * CD.dk <= 0) {
...
void ClothoidCurve::bbox(real_type offs, real_type &xmin, real_type &ymin,
                         real_type &xmax, real_type &ymax) const {
  vector<T2D> tvec;
...
\*/
void ClothoidCurve::build_AABBtree(real_type offs, real_type max_angle,
                                   real_type max_size) const {
  if (aabb_done && isZero(offs - aabb_offs) &&
      isZero(max_angle - aabb_max_angle) && isZero(max_size - aabb_max_size))
    return;
...
#ifdef G2LIB_USE_CXX11
    bboxes.push_back(
        make_shared<BBox const>(xmin, ymin, xmax, ymax, G2LIB_CLOTHOID, ipos));
#else
...
bool ClothoidCurve::collision(ClothoidCurve const &C) const {
...
bool ClothoidCurve::collision(real_type offs, ClothoidCurve const &C,
...
// collision detection
bool ClothoidCurve::approximate_collision(real_type offs,
                                          ClothoidCurve const &C,
                                          real_type offs_C, real_type max_angle,
                                          real_type max_size) const {
...
bool ClothoidCurve::aabb_intersect(T2D const &T1, real_type offs,
                                   ClothoidCurve const *pC, T2D const &T2,
                                   real_type offs_C, real_type &ss1,
                                   real_type &ss2) const {
...
void ClothoidCurve::intersect(real_type offs, ClothoidCurve const &C,
                              real_type offs_C, IntersectList &ilist,
...
void ClothoidCurve::closestPoint_internal(real_type s_begin, real_type s_end,
                                          real_type qx, real_type qy,
                                          real_type offs, real_type &x,
                                          real_type &y, real_type &s,
                                          real_type &dst) const {
#if 1
...
int_type ClothoidCurve::closestPoint(real_type qx, real_type qy, real_type offs,
...
real_type ClothoidCurve::thetaTotalVariation() const {
...
real_type ClothoidCurve::thetaMinMax(real_type &thMin, real_type &thMax) const {
  // cerco punto minimo parabola
  // root = -k/dk;
  real_type kL = CD.kappa0;
  real_type kR = CD.kappa(L);
...
real_type ClothoidCurve::curvatureMinMax(real_type &kMin,
...
real_type ClothoidCurve::curvatureTotalVariation() const {
  // cerco punto minimo parabola
  // root = -k/dk;
...
real_type ClothoidCurve::integralCurvature2() const {
  return L *
         (CD.kappa0 * (CD.kappa0 + L * CD.dk) + (L * L) * CD.dk * CD.dk / 3);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

real_type ClothoidCurve::integralJerk2() const {
  ...
  real_type t3 = L * t2;
  real_type t4 = L * t3;
  return ((((t4 / 5 * CD.dk + t3 * CD.kappa0) * CD.dk + (1 + 2 * t2) * k2) *
               CD.dk +
           2 * t1 * k3) *
              CD.dk +
          k4) *
...
real_type ClothoidCurve::integralSnap2() const {
  real_type k2 = CD.kappa0 * CD.kappa0;
  real_type k3 = CD.kappa0 * k2;
  real_type k4 = k2 * k2;
  ...
  return ((t7 / 7) * dk6 + dk5 * CD.kappa0 * t6 + 3 * dk4 * k2 * t5 +
          5 * dk3 * k3 * t4 + 5 * dk2 * k4 * t3 + 3 * dk3 * t3 +
          3 * CD.dk * k5 * t2 + 9 * dk2 * CD.kappa0 * t2 + k6 +
          9 * k2 * CD.dk) *
         L;
}

void ClothoidCurve::pointsOnClothoid(real_type x0, real_type y0,
                                     real_type theta0, real_type k0,
                                     real_type dk0, real_type L, int num_points,
                                     std::vector<real_type> &sample_x,
                                     std::vector<real_type> &sample_y) const {
  real_type t = L / num_points;
  real_type C, S;
  for (int i = 0; i < num_points; ++i) {
    GeneralizedFresnelCS(dk0 * t * t, k0 * t, theta0, C, S);
    sample_x[i] = x0 + t * C;
    sample_y[i] = y0 + t * S;
    t += L / num_points;
  }
